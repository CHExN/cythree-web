    ...
    <div>
      <div class="operator">
        ...
        <span v-show="deleted===0">
          <a-button v-hasPermission="'staffInside:delete'" @click="batchDelete">删除</a-button>
        </span>
        <span v-show="deleted===1">
          <a-button v-hasPermission="'staffInside:deleteTrue'" @click="batchDelete">删除</a-button>
          <a-button v-hasNoPermission="'staffInside:deleteTrue'" disabled title="无权限">删除</a-button>
        </span>
        <a-dropdown>
          <a-menu slot="overlay">
            <a-menu-item key="export-data" @click="exportExcel" v-hasPermission="'staffInside:export'">导出Excel</a-menu-item>
            ...
            <a-menu-item key="restore" v-if="deleted === 1 && selectedRowKeys.length > 0" @click="restore" v-hasPermission="'staffInside:restore'">恢复</a-menu-item>
          </a-menu>
          <a-button>
  ...
  methods: {
      ...
      this.$confirm({
        ...
        onOk () {
          that.$confirm({
            ...
            onOk () {
              that.loading = true
              that.$put('staffInside/togetherRestore', {
                staffInsideIds: that.selectedRowKeys.join(',')
              }).then(() => {
                that.loading = false
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        onOk () {
          that.loading = true
          that.$delete('staffInside/together', {
            staffInsideIds: that.selectedRowKeys.join(','),
