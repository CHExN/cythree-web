<template>
  <a-drawer
    ...
    :closable="false"
    @close="onClose"
    :visible="staffInsideEditVisiable"
    ...
    <a-form :form="form">
      ...
      <a-form-item label='排序编号' v-bind="formItemLayout">
        <a-input-number
          :placeholder="'填写序号会插入到相应的位置上，不填写不变，当前序号：' + sortNum"
          autocomplete="off"
          style="width: 100%;"
          v-decorator="['sortNum']"
        />
...
export default {
  name: 'StaffInsideEdit',
  components: { DeptInputTree },
  props: {
    staffInsideEditVisiable: {
      default: false
    },
    technicalTypeData: {
      require: true
  ...
  methods: {
    ...
    setFormValues ({...staffInside}) {
      // staffInside.deptId = `${staffInside.deptId}` || ''
      staffInside.deptId = [staffInside.deptId] || []
      this.staffId = staffInside.staffId
      this.sortNum = staffInside.sortNum
      const fields = ['leaveDate', 'birthDate', 'graduationDate', 'workDate', 'farmerWorkDate', 'addDate', 'transferDate', 'technicalLevelDate', 'appointmentDate']
      const excludes = ['sortNum']
      let obj = {}
      Object.keys(staffInside).forEach((key) => {
        if (excludes.includes(key)) return
        this.form.getFieldDecorator(key)
        obj[key] = staffInside[key]
      })
    ...
    handleSubmit () {
      this.form.validateFields((err, values) => {
        if (!err) {
          const {leaveDate, birthDate, graduationDate, workDate, farmerWorkDate, addDate, transferDate, technicalLevelDate, appointmentDate} = values
