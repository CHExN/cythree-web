<template>
  <a-card :bordered="false" class="card-area">
    ...
    <div>
      <div class="operator">
        <a-button @click="batchDelete" v-hasPermission="'reviewDelete:delete'">删除</a-button>
      </div>
      <!-- 表格区域 -->
      <a-table ref="TableInfo"
        ...
        <template slot="operation" slot-scope="text, record">
          <a-icon v-hasPermission="'reviewDelete:view'" type="eye" theme="twoTone" twoToneColor="#42b983" @click="view(record)" title="查看"></a-icon>
          &nbsp;
          <a-popconfirm
            title="是否通过此申请 ?"
            ...
            cancelText="不通过"
          >
            <a-icon v-if="record.process==='0'" v-hasPermission="'reviewDelete:review'" type="check-circle" theme="twoTone" twoToneColor="#9451ff" title="确认"></a-icon>
          </a-popconfirm>
        </template>
      </a-table>
    </div>
...
<script>
import StaffOutsideInfo from '../staffOutside/StaffOutsideInfo'
...
export default {
  name: 'ReviewDelete',
  components: { StaffOutsideInfo, RangeDate, DeptInputTree },
    ...
    columns () {
      ...
      return [{
        ...
        customRender: (text, row, index) => {
          switch (text) {
            case '0':
              return <a-badge color="magenta" text="删除" />
            case '1':
              return <a-badge color="blue" text="修改" />
            default:
              return text
          }
        },
  ...
  methods: {
    ...
    view (record) {
      ...
      this.$get('staffOutside/getStaffOutsideByStaffId', {
        staffId: record.tableId
      }).then((r) => {
        this.$message.destroy()
        this.staffOutsideInfo.data = r.data
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        centered: true,
        onOk () {
          that.$delete('reviewDelete/' + that.selectedRowKeys.join(',')).then(() => {
