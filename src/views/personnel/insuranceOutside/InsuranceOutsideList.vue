<template>
  <a-drawer
    ...
    @close="onClose"
    :closable="false"
    :visible="insuranceOutsideListVisiable"
...
<script>
export default {
  name: 'InsuranceOutsideList',
  props: {
    insuranceOutsideListVisiable: {
      default: false
    }
  },
  data () {
  ...
  methods: {
    determine (record) {
      this.$emit('change', record.name, record.id, record.householdRegistrationType,
        record.pensionInsuranceBase, record.workInjuryInsuranceBase, record.medicalInsuranceBase)
      this.onClose()
    },
    onClose () {
      this.$emit('close')
    ...
    fetch (params = {}) {
      ...
      this.$get('insuranceOutside/insuranceOutsideSimplify', {
        ...params,
  ...
  watch: {
    insuranceOutsideListVisiable () {
      if (this.insuranceOutsideListVisiable) {
        this.fetch()
      }
    }
  }
