<template>
  <a-drawer
    ...
    :closable="false"
    @close="onClose"
    :visible="insuranceInsideEditVisiable"
...
export default {
  name: 'InsuranceInsideEdit',
  components: { StaffInsideList },
  props: {
    insuranceInsideEditVisiable: {
      default: false
    }
  },
  data () {
  ...
  methods: {
    ...
    setFormValues ({...insuranceInside}) {
      this.id = insuranceInside.id
      this.staffId = insuranceInside.staffId
      let fields = ['createTime', 'modifyTime']
      let obj = {}
      Object.keys(insuranceInside).forEach((key) => {
        if (fields.indexOf(key) === -1) {
          this.form.getFieldDecorator(key)
          obj[key] = insuranceInside[key]
        }
      })
