<template>
  <a-card :bordered="false" class="card-area">
    ...
    <div>
      <div class="operator">
        <a-button type="primary" ghost @click="add" v-hasPermission="'taxOutside:add'">新增</a-button>
        <a-button @click="batchDelete" v-hasPermission="'taxOutside:delete'">删除</a-button>
        <a-dropdown v-hasAnyPermission="'taxOutside:export','taxOutside:add'">
          <a-menu slot="overlay">
            <a-menu-item key="download-template" @click="downloadTemplate">模板下载</a-menu-item>
            <a-menu-item key="import-data" v-hasPermission="'taxOutside:add'">
              <a-upload class="upload-area" :fileList="fileList" :beforeUpload="beforeUpload">导入Excel</a-upload>
            </a-menu-item>
            <a-menu-item v-hasPermission="'taxOutside:export'" key="export-data" @click="exportExcel">导出Excel</a-menu-item>
          </a-menu>
          <a-button>
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="operation" slot-scope="text, record">
          <a-icon v-hasPermission="'taxOutside:update'" type="setting" theme="twoTone" twoToneColor="#4a9ff5" @click="edit(record)" title="修改"></a-icon>
          &nbsp;
          <a-icon v-hasPermission="'taxOutside:view'" type="eye" theme="twoTone" twoToneColor="#42b983" @click="view(record)" title="查看"></a-icon>
        </template>
      </a-table>
    </div>
    <!-- 新增编外个税信息 -->
    <tax-outside-add
      :dictData="dictData"
      :taxOutsideAddVisiable="taxOutsideAdd.visiable"
      @close="handleTaxOutsideAddClose"
      @success="handleTaxOutsideAddSuccess">
    </tax-outside-add>
    <!-- 修改编外个税信息 -->
    <tax-outside-edit
      ref="taxOutsideEdit"
      :dictData="dictData"
      :taxOutsideEditVisiable="taxOutsideEdit.visiable"
      @close="handleTaxOutsideEditClose"
      @success="handleTaxOutsideEditSuccess">
    </tax-outside-edit>
    <!-- 编外个税详细信息 -->
    <tax-outside-info
      :taxOutsideInfoData="taxOutsideInfo.data"
      :taxOutsideInfoVisiable="taxOutsideInfo.visiable"
...
export default {
  ...
  methods: {
    ...
    importExcel () {
      ...
      this.$upload('taxOutside/import', formData).then((r) => {
    ...
    add () {
      this.taxOutsideAdd.visiable = true
    },
    handleTaxOutsideAddClose () {
      this.taxOutsideAdd.visiable = false
    },
    handleTaxOutsideAddSuccess () {
      this.taxOutsideAdd.visiable = false
      this.$message.success('新增编外个税信息成功')
      this.search()
    },
    edit (record) {
      this.$refs.taxOutsideEdit.setFormValues(record)
      this.taxOutsideEdit.visiable = true
    },
    handleTaxOutsideEditClose () {
      this.taxOutsideEdit.visiable = false
    },
    handleTaxOutsideEditSuccess () {
      this.taxOutsideEdit.visiable = false
      this.$message.success('修改编外个税信息成功')
      this.search()
    },
    view (record) {
      ...
      this.taxOutsideInfo.data = record
      this.taxOutsideInfo.data.monthArr = this.monthData.slice(0, this.monthData.indexOf(record.month) + 1).join()
      this.taxOutsideInfo.visiable = true
    },
    handleTaxOutsideInfoClose () {
      this.taxOutsideInfo.visiable = false
    },
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        centered: true,
        onOk () {
          that.$delete('taxOutside/' + that.selectedRowKeys.join(',')).then(() => {
