<template>
  <a-drawer
    ...
    :closable="false"
    @close="onClose"
    :visible="taxOutsideEditVisiable"
...
export default {
  ...
  methods: {
    ...
    setFormValues ({...taxOutside}) {
      this.id = taxOutside.id
      this.form.getFieldDecorator('yearMonth')
      this.form.setFieldsValue({yearMonth: moment(`${taxOutside.year}-${taxOutside.month}`)})
      let obj = {}
      let fields = ['createTime', 'modifyTime', 'finalWage']
      Object.keys(taxOutside).forEach((key) => {
        if (fields.includes(key)) return
        this.form.getFieldDecorator(key)
        obj[key] = taxOutside[key]
      })
