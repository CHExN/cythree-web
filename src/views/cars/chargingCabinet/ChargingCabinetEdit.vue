<template>
  <a-drawer
    ...
    :closable="false"
    @close="onClose"
    :visible="chargingCabinetEditVisiable"
...
export default {
  ...
  methods: {
    ...
    setFormValues ({...chargingCabinet}) {
      this.id = chargingCabinet.id
      let fields = ['allotmentDate']
      let forbids = ['createTime', 'modifyTime']
      let obj = {}
      Object.keys(chargingCabinet).forEach((key) => {
        if (forbids.indexOf(key) === -1) {
          this.form.getFieldDecorator(key)
          obj[key] = chargingCabinet[key]
        }
      })
