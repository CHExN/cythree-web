...
export default {
  ...
  methods: {
    setFormValues (put, reviewModify) {
      this.id = put.id
  ...
  watch: {
    putEditVisiable () {
      if (this.putEditVisiable) {
        this.loading = true
        this.$get('unionStoreroomPut/storeroomByPutId', {
          putId: this.id
        }).then((r) => {
          this.loading = false
